...
| Language | Install | Latest | Status |
|----------|---------|--------|--------|
| Python | `pip install "assemblyai>=1.0.0"` | 1.1.0 (Sept 1, 2026) | Active |
| JavaScript/TypeScript | `npm i assemblyai@^4.37.0` | 4.37.1 (Sept 1, 2026) | Active |
...
### Version floors: Python 1.x, Node 4.37+

Target **Python `>=1.0.0`** and **Node `>=4.37.0`** — these are the matched pair. The two SDKs version independently (the JS/TS SDK has been on 4.x since December 2023), so there is no shared version number to match on; match on the release that carries the same change instead. **Python 1.0.0 (Aug 14, 2026)** and **Node 4.37.0 (Aug 28, 2026)** both removed LeMUR, and **Python 1.1.0** and **Node 4.37.1** shipped the same day (Sept 1, 2026) with the same addition — so pinning the current latest of each gives you feature parity.
...
| Python SDK rejects `universal-3-5-pro` | The SDK validates `speech_model` locally against an enum, and pre-`0.64.21` releases omit `universal-3-5-pro`. Install the current floor — `pip install "assemblyai>=1.0.0"` |
